...
class MeshPool(nn.Module):
    ...
    def __call__(self, fe, f_ep, meshes):
        return self.forward(fe, f_ep, meshes)

    def forward(self, fe, f_ep, meshes):
    ...
    def __pool_main(self, mesh_index):
        ...
        """下面这句话中的self.__fe是不是要换为边和点的合并特征？即和上面的用于计算队列的self.__fe不同，这样就行了对么？"""
        # print("多线程中，当前mesh_index的特征用于池化即合并之前的特征 self.__fe.shape:", self.__fe.shape, self.__fe[mesh_index].shape, mesh_index)
        fep = edge_groups.rebuild_features(self.__fep[mesh_index], mask, self.__out_target)
        # print("多线程中，池化后的特征 fep.shape:", fep.shape, mesh_index)
        self.__updated_fe[mesh_index] = fep
